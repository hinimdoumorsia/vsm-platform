  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
      ...
      <p className="text-xs text-gray-400 mt-1 text-center">
        {pce >= 25 ? 'Bon' : pce >= 10 ? 'À améliorer' : 'Critique'} — Lean world class: ≥ 25%
      </p>
    </div>
  );
};
...
const UtilizationChart: React.FC<{ details: NonNullable<ReturnType<typeof useActiveKPIs>>['processDetails'] }> = ({ details }) => {
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" barCategoryGap="40%">
            ...
            <Bar dataKey="value" fill="#3B82F6" radius={[0, 4, 4, 0]}
              label={{ position: 'right', fontSize: 9, formatter: (v: number) => formatDuration(v as number) }}
            />
          </BarChart>
        </ResponsiveContainer>
      </div>
